...
char* Controller::getDataChunk(vector<FileProcess> &fileQueue, int& buffer_size)
{
    int maxFile = fileQueue.size();
    char *buffer = NULL;
    for (int i = 0; i < maxFile; i++)
    {
        ...
        int bufferSize = 0;
        buffer = serializeData(fileQueue[i], bufferSize);
        buffer_size = bufferSize;
        fileQueue[i].process++;
        if (fileQueue[i].process == fileQueue[i].maxProcess)
		{
